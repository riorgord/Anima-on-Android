...
from vae_ops import vae_attention
from vae_ops import Ops as ops


def torch_cat_if_needed(tensors, dim=0):
...
class CausalConv3d(ops.Conv3d):
    ...
    def forward(self, x, cache_x=None, cache_list=None, cache_idx=None):
        ...
        if cache_x is None and x.shape[2] == 1:
            # Single frame: pad temporarily for causal kernel (kernel_t-1 zeros before current frame)
            sp = self.padding  # (0, H_pad, W_pad)
            tp = self._padding  # causal: pad kernel_t-1 zeros before current frame
            x = F.pad(x, (sp[2], sp[2], sp[1], sp[1], tp, 0), mode="constant", value=0)
            saved = self.padding
            self.padding = (0, 0, 0)
            out = super().forward(x)
            self.padding = saved
...
class WanVAE(nn.Module):
    ...
    def __init__(self,
        ...
        # Latent normalization (matching DiffSynth WanVideoVAE)
    ...
    def decode(self, z):
        conv_idx = [0]
        # Latent normalization (matching DiffSynth WanVideoVAE)
        z = z * self.latent_std.to(device=z.device, dtype=z.dtype) \
            + self.latent_mean.to(device=z.device, dtype=z.dtype)
        # z: [b,c,t,h,w]
        iter_ = z.shape[2]
        feat_map = None
        if iter_ > 1:
