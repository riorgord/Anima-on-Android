...
def pt_adaln(emb, lora, block_idx, module):
    ...
    h = F.silu(F.linear(emb, w0))
    out = F.linear(h, w2) + lora
    shift, scale, gate = out.chunk(3, dim=-1)
    return shift, scale + 1.0, gate

# t_emb (same as C++: sin first, cos second)
sigma = 1.0; half = D // 2
...
w_tn = sd['t_embedding_norm.weight']
t_emb_pt = F.rms_norm(emb_pt, (D,), w_tn, 1e-6)
