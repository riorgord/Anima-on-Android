"""Skip-attention pipeline — C++ engine computes lora+t_emb on CPU (no PC pre-compute)."""
...
_lib.dit_compute_timestep.argtypes=[ctypes.c_float]; _lib.dit_compute_timestep.restype=ctypes.c_bool
...
for step in range(3):
    ...
    # C++ CPU computes lora on-device → g_loraBuf (no PC pre-compute! t_emb still from file)
    ok=_lib.dit_compute_timestep(ctypes.c_float(sigma))
    print(f"  compute_lora={ok}")
...
out.astype(np.float16).tofile(f"{OUT}/latent_cpu_lora_final.bin")
print(f"\nTotal: {total_time:.1f}s ({total_time/3:.1f}s/step)")
print("DONE — latent_cpu_lora_final.bin")
_lib.dit_destroy()
