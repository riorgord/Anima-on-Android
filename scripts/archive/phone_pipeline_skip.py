"""Skip-attention pipeline — GPU AdaLN (internal only), V->O skip, no RoPE."""
...
for step in range(3):
    ...
    ext=np.fromfile(f"{OUT}/ext_step{step}.bin",dtype=np.float16)
    _lib.dit_write_buf(9,ext.ctypes.data_as(ctypes.c_void_p),ctypes.c_size_t(ext.nbytes))
...
out.astype(np.float16).tofile(f"{OUT}/latent_skip_final.bin")
print(f"\nTotal: {total_time:.1f}s ({total_time/3:.1f}s/step)")
print("DONE — latent_skip_final.bin")
_lib.dit_destroy()
