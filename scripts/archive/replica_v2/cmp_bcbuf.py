...
def pt_adaln(emb,lora,module):
    ...
    # AdaLN Sequential: SiLU → Linear(D→256) → Linear(256→3D) (no activation between Linears)
    h=F.linear(F.silu(emb),w0)     # SiLU(emb) @ W0^T
    out=F.linear(h,w2)+lora         # h @ W2^T + lora
