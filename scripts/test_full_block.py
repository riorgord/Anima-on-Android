"""Test full block: self-attn + MLP with pre-uploaded AdaLN"""
...
MS,D,M=512,2048,2; torch.manual_seed(42)
...
# Self-attn weights
w_q=sd[pfx+"self_attn.q_proj.weight"]; w_k=sd[pfx+"self_attn.k_proj.weight"]
w_v=sd[pfx+"self_attn.v_proj.weight"]; w_o=sd[pfx+"self_attn.output_proj.weight"]
w_qn=sd[pfx+"self_attn.q_norm.weight"].float(); w_kn=sd[pfx+"self_attn.k_norm.weight"].float()
...
def compute_adaln(emb, w1, w2):
    ...
    scale_p1=scale+1.0
    scale_b=scale_p1.repeat_interleave(MS//M,0)
    shift_b=shift.repeat_interleave(MS//M,0)
    gate_b=gate.repeat_interleave(MS//M,0)
    return scale_b, shift_b, gate_b

scale_s, shift_s, gate_s = compute_adaln(t_emb, wl1_s, wl2_s)
scale_m, shift_m, gate_m = compute_adaln(t_emb, wl1_m, wl2_m)

# Upload AdaLN to bcBuf: [scale_s | shift_s | gate_s | scale_m | shift_m | gate_m]
n_elem=MS*D
...
print("PASS" if err<5.0 else "FAIL")
_lib.dit_destroy()
print("DONE")
