"""Pre-compute LLMAdapter context for phone deployment."""
import sys, torch, safetensors.torch
...
# Qwen
qwen_sd = safetensors.torch.load_file("/mnt/d/AI/手坤的anima/models/text_encoder/qwen_3_06b_base.safetensors")
cfg = Qwen3Config(hidden_size=1024, num_hidden_layers=28, num_attention_heads=16, num_key_value_heads=8,
    intermediate_size=3072, vocab_size=151936, head_dim=128, hidden_act="silu", rms_norm_eps=1e-6,
    rope_theta=1000000.0, attention_bias=False, tie_word_embeddings=True,
    max_position_embeddings=40960, use_cache=False)
qwen = Qwen3Model(cfg).to(DEV).eval()
qwen.load_state_dict({k[6:]: v for k, v in qwen_sd.items()})
del qwen_sd

# LLMAdapter
diff_sd = safetensors.torch.load_file("/mnt/d/AI/手坤的anima/models/diffusion_model/anima-base-v1.0.safetensors")
adapter_sd = {}
...
del adapter_sd, diff_sd

# Tokenizers
qw_tok = Qwen2Tokenizer.from_pretrained("/mnt/d/AI/prompts/画师串小助手/engines/tokenizers/qwen3")
...
for name, text in [("cond", "1girl, anime style, blue eyes"), ("uncond", ".")]:
    ...
    out_path = f"/mnt/d/context_{name}.pt"
