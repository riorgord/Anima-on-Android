...
# transforms stubbed — torchvision version conflict with torch 2.12

import torch.nn.functional as F


def _pad_to_patch_size(img, patch_size, padding_mode="circular"):
    pad = ()
...
def _scaled_dot_product_attention(q, k, v, heads, skip_reshape=False, transformer_options=None):
    if skip_reshape:
        out = F.scaled_dot_product_attention(q, k, v)
        return out.transpose(1, 2).reshape(q.shape[0], -1, heads * q.shape[-1])
    return F.scaled_dot_product_attention(q, k, v).reshape(q.shape[0], q.shape[2], heads * q.shape[-1])
...
def torch_attention_op(q_B_S_H_D: torch.Tensor, k_B_S_H_D: torch.Tensor, v_B_S_H_D: torch.Tensor, transformer_options: Optional[dict] = {}) -> torch.Tensor:
    ...
    return _scaled_dot_product_attention(q_B_H_S_D, k_B_H_S_D, v_B_H_S_D, in_q_shape[-2], skip_reshape=True, transformer_options=transformer_options)
        ...
        if self.concat_padding_mask:
            if padding_mask is None:
                ...
                padding_mask = torch.nn.functional.interpolate(
                    padding_mask, size=list(x_B_C_T_H_W.shape[-2:]), mode="nearest"
                )
