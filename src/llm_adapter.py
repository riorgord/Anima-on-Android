...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        return torch.nn.functional.rms_norm(x, self.weight.shape, weight=self.weight, eps=self.eps)
...
class TransformerBlock(nn.Module):
    def __init__(self, source_dim, model_dim, num_heads=16, mlp_ratio=4.0, use_self_attn=False, layer_norm=False, device=None, dtype=None, operations=None):
        ...
        if self.use_self_attn:
            ...
            self.self_attn = Attention(model_dim, model_dim, num_heads, model_dim // num_heads, device=device, dtype=dtype, operations=operations)
        ...
        self.cross_attn = Attention(model_dim, source_dim, num_heads, model_dim // num_heads, device=device, dtype=dtype, operations=operations)
    ...
    def forward(self, x, context, target_attention_mask=None, source_attention_mask=None, position_embeddings=None, position_embeddings_context=None):
        ...
        x = x + self.cross_attn(normed, mask=source_attention_mask, context=context, position_embeddings=position_embeddings, position_embeddings_context=position_embeddings_context)
...
def adapted_context(adapter, text_embeds, text_ids, t5xxl_weights=None):
    out = adapter(text_embeds, text_ids)
